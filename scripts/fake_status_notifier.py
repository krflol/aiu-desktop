...
def menu_proxy():
    ...
    return Gio.DBusProxy.new_sync(connection, Gio.DBusProxyFlags.NONE, None, service, value.unpack(), "com.canonical.dbusmenu", None)
...
def invoke(label):
    menu = menu_proxy()
    tree = menu.call_sync("GetLayout", GLib.Variant("(uuu)", (0, -1, 0)), Gio.DBusCallFlags.NONE, 3000, None).unpack()[2]
    ...
    menu.call_sync("Event", GLib.Variant("(isvu)", (ident, "clicked", GLib.Variant("()"), 0)), Gio.DBusCallFlags.NONE, 3000, None)
...
for interface in node.interfaces:
    connection.register_object("/org/aiu/Smoke" if interface.name == "org.aiu.Smoke" else "/StatusNotifierWatcher", interface, Gio.DBusInterfaceVTable(method_call), None)
